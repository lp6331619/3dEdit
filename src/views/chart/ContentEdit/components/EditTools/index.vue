<template>
  ...
  >
    ...
    >
      <template #trigger>
        <div class="btn-item">
          <n-button v-if="item.type === TypeEnum.BUTTON" :type="transformControlsState.mode==item.key?'primary':'default'" :circle="isAside" secondary @click="item.handle(item.key)">
            <template #icon>
...
const { DownloadIcon, ShareIcon, PawIcon, SettingsSharpIcon, CreateIcon, ZAxisIcon} = icon.ionicons5
const {StackedMoveIcon} = icon.carbon
const {ScaleFit16Regular,ArrowRotateCounterclockwise24Regular,ArrowMove20Regular} = icon.fluent
...
const setMode = (e:string)=>{
  chartEditStore.setTransformControlsState(e)
}
// 配置列表
