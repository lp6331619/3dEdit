    ...
    >
      <content-box class="go-content-configurations go-boderbox" :show-top="false" :depth="2">
        ...
        <n-tabs v-if="selectTarget" v-model:value="tabsSelect" class="tabs-box" size="small" type="segment">
         ...
         <!-- 页面配置 -->
         <n-tabs v-else class="tabs-box" size="small" type="segment">
          <n-tab-pane
...
const ChartAnimation2 = loadAsyncComponent(() => import('./components/ChartAnimation2/index.vue'))
...
const selectTarget = computed(() => {
  ...
  let target = chartEditStore.getCurrentListItem()
  if (target?.isGroup) {
    // eslint-disable-next-line vue/no-side-effects-in-computed-properties
    // tabsSelect.value = TabsEnum.CHART_SETTING
    target?.groupList?.map((item:any)=>{
      if(item.id==selectId){
        target=item
      }
    })
...
const chartsList = () => {
  const list = [...chartsDefaultTabList]
  if (setDataTypes.includes(selectTarget.value?.key)) {
    list.push({
      key: TabsEnum.CHART_DATA,
      title: '数据',
      icon: FlashIcon,
  ...
  if (selectTarget.value?.type == 'GLTFModel') {
    list.push({
      key: TabsEnum.CHART_ANIMATION,
      title: '动画',
      icon: LeafIcon,
      render: ChartAnimation2
    })
  }
  return list
}
