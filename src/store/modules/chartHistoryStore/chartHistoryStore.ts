...
export const useChartHistoryStore = defineStore({
  ...
  actions: {
    ...
    createStackItem(
      ...
      this.pushBackStackItem(
        Object.freeze({
          ...
          [HistoryStackItemEnum.HISTORY_DATA]: cloneDeep(item),
    ...
    pushBackStackItem(item: HistoryItemType | Array<HistoryItemType>, notClear = false): void {
      if (item instanceof Array) this.backStack = [...cloneDeep(this.backStack), ...cloneDeep(item)]
      else this.backStack.push(cloneDeep(item))
      this.backStack.splice(0, this.backStack.length - editHistoryMax)
      // 新动作需清空前进栈
      console.log(this.backStack,'推入后退栈')
      if (notClear) return
    ...
    // * 初始化组件记录
    createInitHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.INIT, HistoryTargetTypeEnum.CHART)
    },
    // * 新增组件记录
    ...
    // * 缩放组件记录
    createScaleHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.SCALE, HistoryTargetTypeEnum.CHART)
    },
    // * 旋转组件记录
    createRotateHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.ROTATE, HistoryTargetTypeEnum.CHART)
    },
    // * 改变层级组件记录
    createLayerHistory(
