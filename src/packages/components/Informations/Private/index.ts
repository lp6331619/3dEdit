...
import { storedFileUploadFile } from 'swagger-api/export-api/scada-api'
...
const uploadFile = (callback: Function | null = null) => {
  ...
  input.onchange = async () => {
    ...
    reader.onload = () => {
      const eventObj: UploadCompletedEventType = { fileName: name, url: reader.result as string, file: file }
...
const addConfig = {
  ...
  configEvents: {
    ...
    addHandle: (photoConfig: ConfigType) => {
      ...
      //   onPositiveCallback: () => {
      uploadFile(async (e: UploadCompletedEventType) => {
        const res = await storedFileUploadFile({ file: e.file })
